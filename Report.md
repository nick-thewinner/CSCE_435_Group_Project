...
-   Finally, amonst all the sorting algorithms we can safetly assume bitonic and merge are very scalable and able to be parallelized well compared to bubble and quick. This is beacuse both bubble and quick are not inheritly parallel and in our implmentation we did an odd-even bubble and a partitioned quick sort. This makes it so that the increase in thread run into over head much quicker. It was also interesting to see that we though bitonic would be hands down the best but it actually shined in CUDA more while merge takes the cake for MPI. These conclusions are well seen expecailly in our speedup plots.
